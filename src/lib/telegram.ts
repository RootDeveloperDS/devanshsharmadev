...
export function sendTelegramNotification(
  ...
  try {
    // 1. Check environment toggle and credentials synchronously
    ...
    // 3. Gather legal, non-sensitive client metadata immediately before state changes
    ...
    // ⚡ Bolt: Defer non-critical network logic via requestIdleCallback to avoid blocking the main thread during animations or route transitions
    ...
    if (typeof window !== "undefined" && "requestIdleCallback" in window) {
      window.requestIdleCallback(() => { void deferredTask(); });
    } else {
      setTimeout(() => { void deferredTask(); }, 0);
    }
  } catch {
