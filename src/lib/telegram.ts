...
export function sendTelegramNotification(
  ...
  // ⚡ Bolt: Defer telemetry execution to idle time to prevent blocking the main thread during render/navigation
  if (typeof window !== "undefined" && "requestIdleCallback" in window) {
    window.requestIdleCallback(() => void executeTelemetry());
  } else {
    setTimeout(() => void executeTelemetry(), 0);
  }
}
